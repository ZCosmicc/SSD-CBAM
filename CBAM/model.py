...
class SSD300(nn.Module):
    ...
    def __init__(self, n_classes, use_cbam=True):
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        # For each image
        for i in range(batch_size):
            if boxes[i].size(0) == 0:  # If there are no objects in image
                true_classes[i] = 0  # Set all classes to background
                continue
        ...
        conf_loss = (conf_loss_hard_neg.sum() + conf_loss_pos.sum()) / (n_positives.sum().float() + 1e-8)  # 1e-8 to avoid division by zero
