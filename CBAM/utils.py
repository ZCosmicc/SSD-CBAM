...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    ...
    print(f"Reverse label map: {rev_label_map}")

    # Validate labels in detected and true values
    print(f"Unique detected labels: {torch.unique(torch.cat(det_labels))}")
    print(f"Unique true labels: {torch.unique(torch.cat(true_labels))}")
    ...
    # Calculate Average Precisions (APs) for each class (except background)
        ...
        for d in range(n_class_detections):
            ...
            if max_overlap.item() > 0.5:
                if object_difficulties[ind] == 0:
                    original_ind = torch.arange(true_class_boxes.size(0), device=device)[
    ...
    average_precisions = {
        rev_label_map[c + 1] if c + 1 in rev_label_map else f"unknown-{c + 1}": v
...
def find_intersection(set_1, set_2):
    ...
    :return: intersection, a tensor of dimensions (n1, n2)
    """
    if set_1.nelement() == 0 or set_2.nelement() == 0:
        return torch.zeros(set_1.size(0), set_2.size(0))
...
def find_jaccard_overlap(set_1, set_2):
    ...
    :param set_1: set 1, a tensor of dimensions (n1, 4) in boundary coordinates (x_min, y_min, x_max, y_max)
    :param set_2: set 2, a tensor of dimensions (n2, 4) in boundary coordinates (x_min, y_min, x_max, y_max)
    :return: Jaccard Overlap, a tensor of dimensions (n1, n2)
    """
    if set_1.size(0) == 0 or set_2.size(0) == 0:
        return torch.zeros((set_1.size(0), set_2.size(0))).to(set_1.device)
...
def expand(image, boxes, filler):
    ...
    if boxes.nelement() != 0:  # Check if there are any boxes
        ...
        new_boxes = boxes  # If no boxes, return the original empty tensor
...
def random_crop(image, boxes, labels, difficulties):
    ...
    # If there are no boxes, just return the original image and empty tensors
    if boxes.nelement() == 0:
...
def flip(image, boxes):
    ...
    :param image: image, a tensor
    :param boxes: bounding boxes in boundary coordinates, a tensor of dimensions (n_objects, 4)
    :return: flipped image, updated bounding box coordinates
    """
    # Flip image horizontally
    new_image = torch.flip(image, [2])  # Flip along the width dimension (axis 2)
    ...
    if boxes.nelement() != 0:
        ...
        # Flip boxes
        new_boxes = boxes.clone()  # Create a copy to avoid modifying the original tensor
...
def resize(image, boxes, dims=(300, 300), return_percent_coords=True):
    ...
    old_dims = torch.FloatTensor([old_width, old_height, old_width, old_height]).unsqueeze(0)
...
def basic_transform(image, boxes, labels, difficulties, split):
    ...
    if split == 'TRAIN':
        ...
        # Convert PIL image to Torch tensor if not already done
        if not isinstance(new_image, torch.Tensor):
            new_image = FT.to_tensor(new_image)

        # Expand image (zoom out) with a 50% chance - helpful for training detection of small objects
        # Fill surrounding space with the mean of ImageNet data that our base VGG was trained on
        if random.random() < 0.5 and has_boxes:
